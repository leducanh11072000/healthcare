...
public interface HealthInfoRepository   extends JpaRepository<HealthInfo, Long> {
    ...
    @Query(value = "SELECT AVG(e.value) FROM health_info e WHERE e.user_id = ?1 AND e.type = ?2 AND MONTH(e.create_time) = MONTH(?3)\n" +
    ...
    @Query(value = "SELECT AVG(e.value) FROM health_info e WHERE e.user_id = ?1 AND e.type = ?2 AND YEARWEEK(e.create_time) = YEARWEEK(?3);" , nativeQuery = true)
    Double findAvgInWeekByUserIdAndTypeAndCreateTime(Long userId,Long type, LocalDateTime date);
    HealthInfo findTopByAndUserIdAndAndTypeOrderByCreateTimeDesc(Long userId, Long type);
